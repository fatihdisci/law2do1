    ...
    <>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20 bg-card/50 rounded-3xl border border-border/50">
          <p className="text-muted-foreground text-lg">Henüz blog yazısı eklenmedi.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((post) => (
            <article
              key={post.slug}
              className="group bg-card border border-border rounded-2xl p-6 shadow-sm hover:shadow-md hover:border-primary/30 hover:-translate-y-1 transition-all duration-300 flex flex-col"
            >
              {/* Tags */}
              {post.tags.length > 0 && (
              ...
              {/* Meta */}
              <div className="flex items-center gap-3 text-xs text-muted-foreground mb-5 pt-4 border-t border-border">
